...
@Entity
@Table(name = "support_tickets") // JPA 엔티티이며 support_tickets 테이블과 매핑
@Getter
@Setter
@NoArgsConstructor // Lombok: getter/setter, 기본 생성자 자동 생성
public class SupportTicket {
    ...
    public enum Status {
        UPLOADED, ANSWERED
    } // 상태 값: 접수(UPLOADED), 답변완료(ANSWERED)
    ...
    void onCreate() {
        if (createdAt == null)
            createdAt = LocalDateTime.now(); // 영속화 직전 생성 시각 보정
        if (updatedAt == null)
            updatedAt = createdAt; // 최초에는 수정 시각 = 생성 시각
    }
