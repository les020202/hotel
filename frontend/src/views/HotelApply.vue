...
async function onSubmit() {
  ...
  const missing =
    ...
    !gradeLevel.value || !hotelName.value || !address1.value
  ...
  try {
    ...
    banner.value = {
      type: 'success',
      text: '신청이 접수되었습니다. 검토에 2~3일 정도 소요될 수 있습니다.'
    }
    ...
    // 잠깐 보여주고 "내 신청 목록"으로 이동
    setTimeout(() => {
      router.replace('/hotelapply/mine')
    }, 1200)
  } catch (e: any) {
    console.error('hotelapp error raw:', e)
    const status = e?.response?.status
    const data = e?.response?.data
    console.error('hotelapp error parsed:', { status, data })

    const errText =
      data?.error || data?.message || data?.detail || e?.message || '신청 중 오류가 발생했습니다.'
