  ...
  <div class="mx-auto max-w-screen-2xl px-4 py-6 space-y-10">
    ...
    <header class="flex items-start justify-between gap-6">
      ...
      <div class="text-right shrink-0">
        ...
        <div class="text-rose-500 text-2xl font-extrabold">₩{{ money(startingFromTotal) }}</div>
        <div class="mt-2 relative flex items-center justify-end gap-2">
          <!-- ✅ 호텔 찜 -->
          <button
            class="border rounded-xl w-10 h-10 grid place-items-center"
            :title="isHotelWished ? '찜 해제' : '찜하기'"
            @click="toggleHotelWish"
          >
            <svg v-if="isHotelWished" viewBox="0 0 24 24" class="w-6 h-6 fill-rose-500">
          ...
          <!-- 공유 -->
          <div class="relative">
            <button class="border rounded-xl w-10 h-10 grid place-items-center" title="공유하기" @click="shareOpen = !shareOpen">↗</button>
            <div v-if="shareOpen" class="absolute right-0 mt-2 w-44 rounded-xl border bg-white shadow z-10 p-1">
              <button class="w-full text-left px-3 py-2 rounded-lg hover:bg-neutral-100 text-sm" @click="copyUrl">URL 복사</button>
              <button v-if="canShareKakao" class="w-full text-left px-3 py-2 rounded-lg hover:bg-neutral-100 text-sm" @click="shareKakao">카카오톡으로 공유</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="w-full">
        <div class="mx-auto w-full">
          <div class="relative">
              ...
              <div :key="pageIndex" class="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-6">
                <template v-if="pageIndex === 0">
                  <div class="col-span-2 row-span-2 relative overflow-hidden rounded-2xl">
                    ...
                    <img :src="coverImage" alt="메인" class="absolute inset-0 h-full w-full object-cover object-center" loading="eager" />
                  </div>
            ...
            <button v-if="pageCount>1" class="hidden md:grid place-items-center absolute -left-1 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 hover:bg-white shadow" @click="prevPage" aria-label="Previous">‹</button>
            <button v-if="pageCount>1" class="hidden md:grid place-items-center absolute -right-1 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 hover:bg-white shadow" @click="nextPage" aria-label="Next">›</button>

            <div class="absolute -bottom-2 left-1/2 -translate-x-1/2 flex gap-2">
              <button v-for="i in 2" :key="'dot-'+i" @click="goPage(i-1)" class="h-2 w-10 rounded-full" :class="(i-1)===pageIndex ? 'bg-neutral-300' : 'bg-neutral-600'"></button>
            </div>
          </div>
        </div>
      </div>
...
<section class="mt-8">
  ...
  />
    ...
    <section>
      ...
      <div v-if="!loading && offers.length === 0" class="text-gray-500 py-10 text-center">선택한 기간에 판매 가능한 객실이 없습니다.</div>
          ...
          <div class="grid grid-cols-12 gap-4 md:gap-6">
            ...
            <div class="col-span-12 md:col-span-7 flex flex-col">
              <div class="flex items-start justify-between gap-3">
                ...
                <div class="text-right shrink-0">
                  <div class="text-rose-600 text-xl md:text-2xl font-extrabold">₩{{ money(t.priceSum) }}</div>
     ...
     <section class="mt-2" v-show="canShowMap">
      ...
      <div class="relative">
        ...
        <a :href="kakaoLink" target="_blank" rel="noopener" class="absolute inset-0 z-[999] block cursor-pointer" aria-label="카카오맵에서 열기" title="카카오맵에서 열기">
...
function copyUrl () {
  ...
  navigator.clipboard?.writeText(url).then(() => { shareOpen.value = false })
    .catch(() => {
...
const hasCoords = computed(() => hotel.value?.latitude != null && hotel.value?.longitude != null)
const canShowMap = computed(() => hasCoords.value || !!(hotel.value?.address && hotel.value.address.trim()))
...
async function initMapWith (lat, lng) {
  ...
  if (!kakaoMap) { kakaoMap = new maps.Map(mapEl.value, { center, level: 4 }) }
...
async function initMap () {
  ...
  if (Number.isFinite(lat) && Number.isFinite(lng)) { await initMapWith(lat, lng); mapBooting = false; return }
  if (!addr) { mapError.value = '지도를 표시할 주소/좌표가 없습니다.'; mapBooting = false; return }

  const geocoder = new window.kakao.maps.services.Geocoder()
  geocoder.addressSearch(addr, async (result, status) => {
    if (status === window.kakao.maps.services.Status.OK && result?.length) {
      const y = Number(result[0].y); const x = Number(result[0].x)
      await initMapWith(y, x)
    } else { mapError.value = '주소로 좌표를 찾을 수 없습니다.' }
    mapBooting = false
  })
}
...
/* 객실별 로컬 위시(화면 전용) */
const roomWishedIds = ref(new Set())
const isRoomWished = id => roomWishedIds.value.has(id)
function toggleRoomWish(id){ isRoomWished(id) ? roomWishedIds.value.delete(id) : roomWishedIds.value.add(id) }
...
/* ✅ 호텔 찜 토글 (알림 없이) */
const isHotelWished = computed(() => {
  const hid = hotel.value?.id
  return hid ? isWishedHotel(hid) : false
})
async function toggleHotelWish () {
  const hid = Number(hotel.value?.id)
  if (!hid) return
  try {
    await getMe() // 로그인 확인
  } catch {
  ...
  try { await toggleWishlist(hid) } catch (e) {
...
async function goReservation(roomType) {
  ...
  try {
    let userId
    try { const me = await getMe(); userId = me.id }
    catch {
      const redirect = encodeURIComponent(location.pathname + location.search)
      router.push(`/login?redirect=${redirect}`)
      return
