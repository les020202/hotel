...
/* ---------------------- 서버 시간 동기화(요구 2) ---------------------- */
const serverOffsetMs = ref(0) // 서버 now - 클라이언트 now
async function syncServerTime() {
  try {
    // 백엔드에 /api/time(예: {"now": "2025-09-20T00:31:11+09:00"})가 있으면 사용
    const api = import.meta.env.VITE_API_BASE
    if (api) {
      const r = await fetch(`${api}/api/time`, { cache: 'no-store' })
      if (r.ok) {
        const j = await r.json()
        const serverNow = Date.parse(j.now)
        const localNow  = Date.now()
        if (!Number.isNaN(serverNow)) serverOffsetMs.value = serverNow - localNow
        return
      }
    }
    // 없으면 Date 헤더 활용 (동일 도메인/프록시 환경)
...
let timerId = null
function nowMs() { return Date.now() + serverOffsetMs.value } // 서버 기준
function tick () {
...
async function cancelHold() {
  ...
  try {
    ...
    holdCode.value = ''
    const hotelIdNum = Number(route.query.hotelId || route.params.id)
    const qCheckIn   = String(route.query.checkIn || checkIn.value || '')
    const qCheckOut  = String(route.query.checkOut || checkOut.value || '')
    const guestsN    = Number(route.query.guests || guests.value || 1)
    router.replace({ name: 'hotel-detail', params: { id: hotelIdNum }, query: { checkIn: qCheckIn, checkOut: qCheckOut, guests: guestsN } })
  } catch (e) {
    console.error(e); alert(e?.response?.data?.message ?? '홀드 취소에 실패했습니다.')
  } finally { cancelling.value = false }
}

/* ---------------------- 가격/쿠폰 ---------------------- */
const price = reactive({ base: quotedTotal.value ?? 240000, taxes: quotedTotal.value ? 0 : 24000, fee: quotedTotal.value ? 0 : 5000 })
const coupons = ref([
...
const total             = computed(()=> isServerQuoted.value ? (quotedTotal.value ?? 0) : (subTotal.value - discount.value))
...
const isCardSelected = computed(()=> cards.value.some(c => c.selected)) // 요구 6
const selectCard = (id)=> cards.value.forEach(c=>c.selected = c.id===id)

/* ---------------------- 카드 추가 모달(생략: 기존 코드 유지) ---------------------- */
...
function detectBrand(num){ const n = num.replace(/\s/g,''); if(/^4/.test(n)) return 'VISA'; if(/^5[1-5]/.test(n)) return 'Mastercard'; if(/^3[47]/.test(n)) return 'AMEX'; if(/^6(?:011|5)/.test(n)) return 'Discover'; return 'CARD' }
function validateCard(){ errors.number=errors.exp=errors.cvc=errors.name=''; const digits = newCard.number.replace(/\s/g,''); if(!/^\d{16}$/.test(digits)) errors.number='카드 번호 16자리를 입력하세요.'; const m = newCard.exp.match(/^(\d{2})\/(\d{2})$/); if(!m) errors.exp='MM/YY 형식으로 입력하세요.'; else { const mm=+m[1]; if(mm<1||mm>12) errors.exp='월(MM)이 올바르지 않습니다.' } if(!/^\d{3,4}$/.test(newCard.cvc)) errors.cvc='CVC 3~4자리를 입력하세요.'; if(!newCard.name.trim()) errors.name='카드 소유자 이름을 입력하세요.'; return !(errors.number||errors.exp||errors.cvc||errors.name) }
async function submitCard(){ if(!validateCard()) return; saving.value = true; try{ const id = Date.now(); const last4 = newCard.number.replace(/\D/g,'').slice(-4); const brand = detectBrand(newCard.number); cards.value = cards.value.map(c=>({ ...c, selected:false })); cards.value.unshift({ id, brand, last4, exp:newCard.exp, selected:true }); showAdd.value = false } finally { saving.value = false } }
...
async function setupToss() {
  try {
    ...
    paymentMethodsWidget = await paymentWidget.renderPaymentMethods(
      '#' + paymentMethodElId,
      { value: total.value, currency: 'KRW' },
      { variantKey: 'DEFAULT' }
    )
    await paymentWidget.renderAgreement('#' + agreementElId, { variantKey: 'AGREEMENT' })
  } catch (e) {
...
/* ---------------------- 결제 버튼 상태/로딩(요구 3·4·9·10) ---------------------- */
const isPaying = ref(false)
const canPay = computed(() =>
  !expired.value &&
  validateGuest() &&               // 입력 검증 통과
  isCardSelected.value &&          // 카드 선택됨
  !!paymentWidget                  // 위젯 준비됨
)
...
async function requestPay() {
  ...
  try {
    ...
    await paymentWidget.requestPayment({
      ...
      failUrl:    `${window.location.origin}/reservation/fail?${extra}`,
      customerName: guest.name || '예약고객',
      customerMobilePhone: guest.phone.replace(/\D/g, ''),
      customerEmail: 'guest@example.com'
    })
  } catch (e) {
    toast(`결제 시작 실패: ${e?.message || '알 수 없는 오류'}`) // 요구 10
  } finally {
...
watch(total, async (v) => {
  if (paymentMethodsWidget) {
    try { await paymentMethodsWidget.updateAmount({ value: v, currency: 'KRW' }) }
    catch (e) { console.warn('amount 업데이트 실패:', e) }
  }
})
...
onMounted(async () => {
  ...
  await syncServerTime() // 서버 시간 먼저 맞추고
  startTimer()
...
const genOrderId = () => {
  ...
  const rand = Math.random().toString(36).slice(2, 10).toUpperCase()      // 8자
  // holdCode가 있으면 접두 일부를 붙여 추적성↑ (선택)
  const holdFrag = (typeof holdCode !== 'undefined' && holdCode?.value)
    ? String(holdCode.value).replace(/[^A-Z0-9]/gi, '').slice(-4).toUpperCase()
    : null
  return holdFrag ? `ORDER-${ts}-${holdFrag}-${rand}` : `ORDER-${ts}-${rand}`
}
...
+   <button class="btn sm outline" @click="handleOpenCoupon">쿠폰 선택</button>
        ...
        <div class="panel guest">
          ...
          <div class="row gap">
            <div class="col">
              ...
              <input class="input"
                     :class="{invalid: !!(guestErr && guestErr.name)}"
            ...
            <div class="col">
              ...
              <input class="input"
                     :class="{invalid: !!(guestErr && guestErr.phone)}"
        ...
        <div class="panel">
          ...
          <!-- 요구 1,3,4,10: 금액 노출 + 조건부 활성 + 로딩 -->
          <button class="btn primary" style="margin-top:12px"
                  :disabled="!canPay || isPaying"
                  @click="requestPay">
            <span v-if="!isPaying">
              {{ expired ? '만료됨' : `₩${total.toLocaleString('ko-KR')} 결제하기` }}
            </span>
...
.card-row.active{background:#e7f0ff;border-color:#5aa0ff} /* 요구 6: 진한 표시 */
...
.btn.add{width:100%;height:48px;background:#2563eb;color:#fff;font-weight:800;border:1px solid #1d4ed8;border-radius:10px}
