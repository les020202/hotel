...
const widgetReady = ref(false)  // ✅ 렌더 완료 신호
...
await nextTick(); // ✅ 컨테이너 DOM 보장
   const pw = await loadPaymentWidget(clientKey, customerKey)
   const amt = krw(payAmount.value)
   // ✅ 객체 형태 { value } 권장
   const pmw = await pw.renderPaymentMethods('#' + paymentMethodElId, { value: amt }, { variantKey: 'DEFAULT' })
   await pw.renderAgreement('#' + agreementElId, { variantKey: 'AGREEMENT' })
   paymentWidget.value = pw
   paymentMethodsWidget.value = pmw
   await ensureWidgetAmountUpToDate()
  // ✅ 내부 iframe 레이아웃이 잡히도록 한 틱 대기
...
async function requestPay() {
  if (!canPay.value) {
    if (!widgetReady.value) toast('결제 UI 준비 중입니다. 잠시만요!')
    return
  }
  isPaying.value = true
  try {
