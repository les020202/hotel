...
onMounted(async () => {
  ...
  // URL > localStorage 순서로 holdCode 확보
  let holdCode = params.get('holdCode') || localStorage.getItem('holdCode') || ''

  // 실패 시 자동으로 홀드 정리(재고 복구). 멱등이므로 실패는 무시
  if (holdCode && !didCancel) {
    try {
      ...
      // 항상 정리
      if (localStorage.getItem('holdCode') === holdCode) {
