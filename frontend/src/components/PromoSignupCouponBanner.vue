  ...
  >
    ...
    <div class="promo-left">
      ...
      <div class="ctas" v-else>
        <button class="btn primary" @click="goBenefits">쿠폰함 보기</button>
      </div>
    </div>
...
import { getUser, onAuthChanged } from '@/api/auth'   // ✅ 로그인 상태 연동
...
let off = null
onMounted(() => {
  off = onAuthChanged((e) => { user.value = e.detail.user })
})
onUnmounted(() => { off?.() })
