...
import { hotelsSearch, fetchMinPrices } from '@/api/searchApi'
...
.body { padding:12px 14px 16px; flex:1; display:flex; flex-direction:column; gap:6px; }
.name { margin:0 0 6px; font-size:16px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.sub { margin:0; color:#777; font-size:13px; line-height:1.3;
       display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
.price { margin-top:auto; padding-top:10px; border-top:1px solid #f2f2f2; font-size:15px; }
.price strong { font-size:20px; font-weight:800; }
@media (max-width:1280px){ .grid{ grid-template-columns: repeat(4, 1fr); } }
@media (max-width:1000px){ .grid{ grid-template-columns: repeat(3, 1fr); .rand-wrap{ --card-h: 300px; } } }
@media (max-width:700px){ .grid{ grid-template-columns: repeat(2, 1fr); .rand-wrap{ --card-h: 290px; } } }
@media (max-width:480px){ .grid{ grid-template-columns: 1fr; .rand-wrap{ --card-h: 280px; } } }
</style>
